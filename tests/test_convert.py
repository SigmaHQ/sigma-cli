...
def test_convert_output_list_of_str():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-p", "another_test", "--disable-pipeline-check", "tests/files/valid"])
...
def test_convert_stdin():
    ...
    result = cli.invoke(convert, ["-t", "text_query_test", "-p", "another_test", "--disable-pipeline-check", "-"], input=input)
    assert 'EventID=1 and ParentImage endswith "\\httpd.exe" and Image endswith "\\cmd.exe"' in result.stdout

def test_convert_output_list_of_dict():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-f", "list_of_dict", "tests/files/valid"])
    assert 'ParentImage' in result.stdout

def test_convert_output_list_of_dict_indent():
    cli = CliRunner()
    result_noindent = cli.invoke(convert, ["-t", "text_query_test", "-f", "list_of_dict", "tests/files/valid"])
    result_indent = cli.invoke(convert, ["-t", "text_query_test", "-f", "list_of_dict", "-j", "2", "tests/files/valid"])
    assert len(result_indent.stdout.split("\n")) > len(result_noindent.stdout.split("\n"))

def test_convert_output_str():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-f", "str", "tests/files/valid"])
    assert 'ParentImage' in result.stdout
...
def test_convert_output_bytes(tmp_path):
    ...
    result = cli.invoke(convert, ["-t", "text_query_test", "-f", "bytes", "-o", str(test_file), "tests/files/valid"])
    assert result.exit_code == 0
...
def test_convert_unknown_format():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-f", "nonexisting", "tests/files/valid"])
...
def test_convert_unknown_pipeline():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-p", "nonexisting", "tests/files/valid"])
...
def test_convert_missing_pipeline_ignore():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "mandatory_pipeline_test", "--without-pipeline", "tests/files/valid"])
    assert 'ParentImage' in result.stdout

def test_convert_wrong_pipeline():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-p", "another_test", "tests/files/valid"])
...
def test_yml_pipeline_doesnt_trigger_wrong_pipeline():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-p", "dummy_test", "-p", "tests/files/custom_pipeline.yml", "tests/files/valid"])
...
def test_backend_option_unknown_by_backend():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-O", "unknown=parameter", "tests/files/valid"])
    assert result.exit_code != 0
    assert "Parameter 'unknown' is not supported" in result.stdout

def test_convert_output_backend_option():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-f", "list_of_dict", "-O", "testparam=testvalue", "tests/files/valid"])
    assert 'testvalue' in result.stdout

def test_convert_output_backend_option_list():
    cli = CliRunner()
    result = cli.invoke(convert, ["-t", "text_query_test", "-f", "list_of_dict", "-O", "testparam=123", "-O", "testparam=test", "tests/files/valid"])
