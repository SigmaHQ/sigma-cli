...
def test_check_pysigma_version_incompatible(
    monkeypatch, pysigma_expected_version, pysigma_installed_version, expected_result
):
    monkeypatch.setattr(
        "importlib.metadata.requires",
        lambda x: [f"pysigma ({pysigma_expected_version})"],
    )
    monkeypatch.setattr(
        "importlib.metadata.version", lambda x: pysigma_installed_version
    )
